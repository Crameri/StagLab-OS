...
%%                                         StagLab PARAMETER FILE: YINYANG
...
clearvars -except iLoop SAVE; if ~exist('f_Defaults','file'); f_INSTALL; end
    ...
    %% INPUT FILE(S)
    IN.Name                     =   { 'testyy'       'testyy'};      	%Filename (it's size controls the # of files processed)
    IN.Number                   =   [   iLoop        1 ];             	%Filenumber (use iLoop to loop through multiple files)
