...
%%                                             StagLab PARAMETER FILE: 2-D - FLUIDITY
%                              (for all available options see f_Defaults.m)
clear; clf;
SAVE.StartNumber       =   1;      %Set SAVE.StartNumber==SAVE.EndNumber to write only one file
SAVE.StepNumber        =   1;
SAVE.EndNumber         =   1;      %Set big in order to continue to last file
...
clearvars -except iLoop SAVE; if ~exist('f_Defaults','file'); f_INSTALL; end
    ...
    %% DEBUGGING
    SWITCH.Verbose              =   logical(1);                         %Switch for extended display output
    SWITCH.sendErrorLog         =   logical(0);                         %Sends an error log file to the developer for debugging purposes

    %% INPUT FILE(S)
    IN.Name                     =   { 'example_csv_file_with_free_surface' }; 	%Filename (it's size controls the # of files processed)
    IN.Number                   =   [   iLoop  	];                              %Filenumber (use iLoop to loop through multiple files)
    IN.Parameter                =   [  	111     ];                              %Parameter dimensionalisation (see f_Dimensions)
    IN.Folder                   =   {'~/work/plotTest/FluidityOutput/'};         %File directory
    %PLOT.titleString            =   {'XXX'          'XXX'};                    %Adds manual titles if uncommented

    %% StagLab MODES
    SWITCH.GeodynamicCode       =   'Fluidity';                           %To use StagLab with a different code, run and adjust at locations of error messages
    ...
    SWITCH.ReverseLayout       	=   true;           %e.g., plot 3x2 instead of 2x3
    ...
    PLOT.Composition            =   logical(0);  SWITCH.TickLabelsComposition = {'Crust','Mantle','Continent','Primordial'};
    ...
    PLOT.Topography           	=   logical(1);  TOPO.cmb=logical(0); TOPO.surf=logical(1);
        SWITCH.plotTOPOtrue     =   logical(0);
     	SWITCH.plotTOPOsmooth   =   logical(1);
        TOPO.indicateComponents = 	logical(1);     %Indication for isostatic and residual topography 
    ...
    %% GRAPHS TO PLOT
    PLOT.PlateVelocity         	=   logical(1);     %Graph indicating horizontal plate velocity
    ...
    PLOT.PlateSketch            =   logical(1);     %Plot sketch of plates with subduction parameters indication
    ...
    IN.streamfun_eta            =   logical(1);  IN.streamNumContours = 17;
    ...
    IN.lithoThickness_eta       =   logical(0);  IN.lithoTisovalue 	= 1400; 
    IN.lithoThickness_nstr      =   logical(0);  IN.lithozmax     	= 500;
    
    IN.fieldContour_T           =   logical(0);  PLOT.fContourField	= 'Temperature';
    IN.fieldContour_eta         =   logical(0);  IN.fContourIsovalue= 1400;
    ...
    TOPO.field                  =   logical(1);     %Add topography to field plots
        TOPO.line               =   logical(1); 
    ...
    SAVE.GeodynamicDiagnostics   	=   logical(0);     %Saves tectonic data (see list of data in SL_FieldPlot.m)
