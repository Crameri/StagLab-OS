...
if strcmp(Type, '2D'),
    ...
    %----------------------------------------
    state(1,:) = a*cos(theta); %max(state(1,:)) seems to be the minor
    state(2,:) = b*sin(theta); %max(state(2,:)) seems to be the major

    %fabio:
    angle_minor = theta(state(1,:)==max(state(1,:))); %in [rad] ~0 ?
    angle_major = theta(state(2,:)==max(state(2,:))); %in [rad]  ~90 ?
